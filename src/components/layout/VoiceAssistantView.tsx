    ...
    <div className="h-full flex flex-col items-center justify-between p-8 relative overflow-hidden bg-slate-950">
      ...
      <div className="flex items-center gap-4 z-10 w-full max-w-4xl justify-between">
        <div className="flex items-center gap-2 bg-slate-900/80 backdrop-blur border border-slate-800 px-4 py-2 rounded-full">
          <Wifi
            size={16}
            className={connectionState === 'connected' ? 'text-green-500' : 'text-slate-500'}
          />
      ...
      <div className="flex-1 flex items-center justify-center relative z-10 w-full">
        <div className="relative flex items-center justify-center">
          <div
            className={`absolute w-[500px] h-[500px] border border-[#00aed9]/10 rounded-full transition-all duration-1000 ${isSpeaking ? 'scale-110 opacity-50' : 'scale-100 opacity-20'}`}
          />
          <div
            className={`absolute w-[400px] h-[400px] border border-[#00aed9]/20 rounded-full transition-all duration-700 ${isSpeaking ? 'scale-105 opacity-60' : 'scale-100 opacity-30'}`}
          />
          <div
            className={`absolute w-[300px] h-[300px] border border-[#00aed9]/30 rounded-full transition-all duration-500 ${isListening ? 'scale-110 border-green-500/30' : 'scale-100'}`}
          />
      ...
      <div className="w-full max-w-3xl z-10 mb-8">
        ...
        >
          {transcriptions.length === 0 && connectionState === 'connected' && (
            <p className="text-center text-slate-500 animate-pulse mt-20">
