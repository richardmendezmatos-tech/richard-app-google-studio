  ...
  .action(async () => {
    ...
    sync.on('close', (code) => {
      if (code === 0) {
        spinner.succeed(
          chalk.green(
            'Sincronización completada. El inventario está actualizado en todos los canales.',
          ),
        );
      } else {
        spinner.fail(
          chalk.red('La sincronización ha fallado. Verifica la conectividad de Sentinel.'),
        );
        process.exit(1);
      }
    });
