...
export const marketingCommand = new Command('marketing')
  ...
  .action(async (vin) => {
    ...
    if (vin) {
      ...
      console.log(chalk.yellow(`💡 No se especificó VIN. Seleccionando unidad destacada: ${car.name}`));
    }

    // DEBUG: check env
    console.log(chalk.gray(`[DEBUG] Google Key: ${process.env.GOOGLE_GENERATIVE_AI_API_KEY ? 'Presente' : 'MISSING'}`));
    console.log(chalk.gray(`[DEBUG] Vite Key: ${process.env.VITE_GEMINI_API_KEY ? 'Presente' : 'MISSING'}`));
...
Badge: ${car.badge || 'N/A'}`;
      ...
      console.log(chalk.gray('Tip: Usa este copy en Facebook Marketplace o ClasificadosOnline para maximizar leads.'));
    } catch (error) {
