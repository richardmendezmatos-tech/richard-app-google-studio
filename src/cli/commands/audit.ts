...
export const auditCommand = new Command('audit')
  ...
  .action(async () => {
    ...
    audit.on('close', (code) => {
      if (code === 0) {
        spinner.succeed(
          chalk.green('Auditoría superada. La arquitectura es resiliente y escalable.'),
        );
      } else {
        spinner.fail(
          chalk.red(
            'Se detectaron violaciones estructurales. Corrige los imports antes de continuar.',
          ),
        );
        process.exit(1);
      }
