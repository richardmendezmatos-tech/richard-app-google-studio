...
export const distributeCommand = new Command('distribute')
  ...
  .option(
    '--platform <platform>',
    'Target a specific platform (clasificados_online, facebook_marketplace)',
  )
  ...
  .action(async (options) => {
      ...
      try {
        ...
        for (const car of initialInventoryData) {
          ...
          if (statuses.length === 0) {
            ...
            console.log(
              `  ${car.vin.slice(0, 17)} | facebook_marketplace  | ${chalk.yellow('none     ')} | Nunca`,
            );
            console.log(
              `  ${car.vin.slice(0, 17)} | clasificados_online   | ${chalk.yellow('none     ')} | Nunca`,
            );
          }
          for (const s of statuses) {
            const statusColor =
              s.status === 'active' ? chalk.green : s.status === 'error' ? chalk.red : chalk.yellow;
            console.log(
              `  ${car.vin.slice(0, 17)} | ${s.platform.padEnd(21)} | ${statusColor(s.status.padEnd(9))} | ${s.lastSync || 'Nunca'}`,
            );
          }
        }
    ...
    console.log(
      chalk.yellow('Uso: sentinel distribute --status o sentinel distribute --vin <VIN>'),
    );
  });
