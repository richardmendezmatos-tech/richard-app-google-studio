...
const getSender = () => ({
  email:
    process.env.SENDGRID_FROM_EMAIL || process.env.SMTP_USER || 'richard@richardautomotive.com',
...
const trySmtpFallback = async (to: string, subject: string, html: string, text?: string) => {
  ...
  console.log(`✅ SMTP fallback email sent to ${to}`, {
 ...
 */
export const sendPlainEmail = async (to: string, subject: string, text: string, html?: string) => {
  const isConfigured = ensureSendGridConfigured();
  if (!isConfigured) {
  ...
  try {
    ...
    console.error(`❌ Error sending plain email to ${to}:`, error.response?.body || error);
    return trySmtpFallback(to, subject, html || text, text);
  }
};
