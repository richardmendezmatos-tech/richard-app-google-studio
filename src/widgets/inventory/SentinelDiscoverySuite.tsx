        ...
        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
          <div className="text-center">
            ...
            <p className="text-slate-500 text-xs mt-2 max-w-lg mx-auto leading-relaxed">
              Nuestro motor neural analiza el inventario en tiempo real para encontrar unidades que no solo encajen con tu bolsillo, sino con tu estilo de vida.
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {neuralResults.map(car => (
                  <div
                    key={car.id}
                    onClick={() => navigate(`/inventario/${generateVehicleSlug(car)}/${car.id}`)}
                    className="group flex gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-indigo-500/40 hover:bg-indigo-500/5 transition-all cursor-pointer overflow-hidden relative"
                  >
                    ...
                    <div className="flex flex-col justify-center">
                      <span className="text-[8px] font-tech text-indigo-400 uppercase tracking-widest mb-0.5">{car.year} • {car.type}</span>
