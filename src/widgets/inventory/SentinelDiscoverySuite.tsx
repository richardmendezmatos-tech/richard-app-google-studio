...
import { Sparkles, Sliders, Flame, Heart, X, RotateCcw, DollarSign, Activity, ChevronRight, Timer, CheckCircle2, User, Mail, Phone, Gift, Send } from 'lucide-react';
...
const SentinelDiscoverySuite: React.FC<SentinelDiscoverySuiteProps> = ({ inventory }) => {
  ...
  const [activeTab, setActiveTab] = useState<'budget' | 'swipe'>('budget');
  ...
  return (
    <div className="w-full rounded-4xl border border-white/10 bg-linear-to-b from-slate-900/90 via-slate-950/95 to-slate-900/90 p-6 md:p-8 backdrop-blur-2xl shadow-[0_0_50px_rgba(34,211,238,0.1)] relative overflow-hidden mb-12">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/10 pb-6 mb-8 relative z-10">
        ...
        <div className="flex bg-slate-950/80 p-1 rounded-full border border-white/10 w-full md:w-auto grid grid-cols-2 md:flex gap-1">
          <button
            onClick={() => setActiveTab('budget')}
            className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider transition-all duration-300 ${
              activeTab === 'budget'
                ? 'bg-linear-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/25'
                : 'text-slate-400 hover:text-white hover:bg-white/5'
            }`}
          >
            <Sliders size={14} /> Presupuesto
          </button>
          <button
