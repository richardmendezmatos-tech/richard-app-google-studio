...
const DealSheet = React.forwardRef<HTMLDivElement, DealSheetProps>(({ lead, car }, ref) => {
  ...
  return (
    <div ref={ref} className="p-12 max-w-[210mm] mx-auto bg-white text-slate-900 print:p-0">
      {/* Header */}
      <header className="flex justify-between items-end border-b-4 border-primary pb-6 mb-10">
        <div>
          <div className="flex items-center gap-2 text-primary font-black text-sm uppercase tracking-[0.2em] mb-2">
            Richard Automotive
          </div>
          <h1 className="text-4xl font-black uppercase tracking-tighter text-slate-900 leading-none">
            Hoja de <span className="text-primary">Trato</span>
          </h1>
        </div>
          ...
          {car ? (
            <>
              <div>
                ...
                <div className="text-xl font-black text-[#173d57]">
                  {car.year || '2024'} {car.name}
                </div>
              </div>
              ...
              {(car.image || car.img || car.images?.[0]) && (
                <div className="mt-4 border border-slate-200 rounded-xl overflow-hidden h-32 w-full">
                  <img
                    src={car.image || car.img || car.images?.[0]}
                    alt={car.name}
