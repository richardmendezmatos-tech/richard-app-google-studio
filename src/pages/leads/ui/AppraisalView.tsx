  ...
  >
    <div className="text-center mb-8">
      ...
      <p className="text-slate-500 text-sm mt-2">
        Dinos a dónde enviar el reporte de IA y desbloquea tu oferta final.
      </p>
    </div>
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <input
          ...
          value={info.name}
          onChange={onChange}
          className="w-full bg-slate-100 dark:bg-slate-800 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-primary font-medium"
...
const InfoStep = React.memo(({ info, onChange, onNext }: any) => (
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="space-y-2">
        ...
        <input
          ...
          value={info.year}
          onChange={onChange}
          className="w-full bg-slate-100 dark:bg-slate-700 p-3 rounded-xl outline-none focus:ring-2 focus:ring-primary"
          placeholder="2018"
        />
      </div>
      <div className="space-y-2">
    ...
    <button
      onClick={onNext}
      className="w-full py-4 bg-primary text-white rounded-xl font-bold uppercase tracking-widest shadow-lg shadow-cyan-500/20 active:scale-95 transition-transform flex items-center justify-center gap-2"
    >
...
const PhotoStep = React.memo(({ photos, onUpload, progress, count, onNext }: any) => (
  ...
  >
    <div className="flex justify-between items-center mb-2">
      ...
      <span
        className={`text-xs font-black uppercase px-2 py-1 rounded-lg ${count === 4 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
      >
        Fotos: {count}/4
      </span>
    </div>
    ...
    <div className="appraisal-progress-container h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-6">
      <div
        className={`h-full transition-all duration-500 ${
          count === 4 ? 'bg-emerald-500 progress-filled' : 'bg-primary progress-active'
        } ${
          count === 0
            ? 'w-0'
            : count === 1
      ...
      {Object.keys(photos).map((key) => (
        ...
        >
          {photos[key] ? (
            ...
            <>
              <Camera
                className="text-slate-400 mb-2 group-hover:text-primary group-hover:scale-110 transition-all"
                size={24}
              />
    ...
    <button
      onClick={onNext}
      disabled={count < 4}
      className="w-full py-4 bg-primary disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-xl font-bold uppercase tracking-widest shadow-lg shadow-cyan-500/20 active:scale-95 transition-all flex items-center justify-center gap-2"
    >
  ...
  >
      ...
      {analysis && (
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-bold px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-700 uppercase text-xs">
              {analysis.condition}
            </span>
          </div>
          <div>
          ...
          {analysis.defects.length > 0 && (
            <div>
              ...
              <ul className="list-disc list-inside text-rose-500 font-medium">
                {analysis.defects.map((d: string, i: number) => (
                  <li key={i}>{d}</li>
                ))}
              </ul>
            </div>
    ...
    <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-2xl text-left space-y-4">
      ...
      <div className="flex justify-between">
        ...
        <span className="font-bold text-primary">7 Días</span>
      </div>
    </div>
    ...
    <div className="space-y-3">
      ...
      <button
        onClick={onCancel}
        className="w-full py-4 text-slate-400 font-bold uppercase tracking-widest text-xs hover:text-slate-600"
      >
        Lo pensaré
      </button>
    </div>
...
const AppraisalView: React.FC = () => {
  ...
  const handleExpressCapture = useCallback(async () => {
    ...
    try {
      const { addLead } = await import('@/shared/api/adapters/leads/crmService');
      await addLead({
        type: 'trade-in',
    ...
    <div className="min-h-screen bg-white dark:bg-slate-900 p-6 pb-24">
      ...
      <header className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-black uppercase italic tracking-tighter">
          Vender mi <span className="text-primary">Auto</span>
        </h1>
      </header>
