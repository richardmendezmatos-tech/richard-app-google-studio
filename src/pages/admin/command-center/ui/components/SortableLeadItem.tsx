...
const SortableItemContainer = memo(({ lead, userRole, onOpenDealSheet, onOpenInbox, onOpenDetail }: SortableLeadItemProps) => {
  const { 
    attributes, 
    listeners, 
    setNodeRef, 
  ...
  React.useLayoutEffect(() => {
    if (innerRef.current) {
      innerRef.current.style.setProperty('--translate', CSS.Translate.toString(transform) || 'none');
