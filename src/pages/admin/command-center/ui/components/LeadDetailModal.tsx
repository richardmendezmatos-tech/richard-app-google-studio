...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ lead, onClose }) => {
  ...
  const handleRevealSSN = async () => {
    ...
    const masterKey = window.prompt('Ingresa la Master Key para acceder a este SSN (Richard Intelligence Vault):');
    if (!masterKey) return;
        ...
        >
          ...
          <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/2">
            <div className="flex items-center gap-5">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${isFinance ? 'bg-indigo-500/20 text-indigo-400' : 'bg-cyan-500/20 text-cyan-400'}`}>
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${isFinance ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'}`}>
                    {isFinance ? 'Credit Application' : 'General Lead'}
                  </span>
                </div>
                <p className="text-slate-500 text-xs font-bold flex items-center gap-2">
                  <Calendar size={12} /> {lead.createdAt?.seconds ? new Date(lead.createdAt.seconds * 1000).toLocaleDateString() : 'Fecha Desconocida'}
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <InfoRow icon={Mail} label="Email Address" value={lead.email} color="text-cyan-400" />
                <InfoRow icon={Phone} label="Mobile Number" value={lead.phone} color="text-emerald-400" />
                
                <div className="p-4 bg-white/3 rounded-2xl border border-white/5 relative overflow-hidden">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Secure Vault (SSN)</p>
                    <button
                    ...
                    >
                      {isRevealing ? <Loader2 size={10} className="animate-spin" /> : revealedSSN ? <EyeOff size={10} /> : <Eye size={10} />}
                  ...
                  {revealedSSN && <ShieldCheck size={14} className="absolute bottom-4 right-4 text-cyan-500" />}
                </div>
                ...
                <div className="mt-8">
                  ...
                  <div className="p-5 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-2xl border border-cyan-500/10">
                    <h4 className="text-lg font-black text-white mb-2">{lead.vehicleOfInterest || 'No Unit Selected'}</h4>
