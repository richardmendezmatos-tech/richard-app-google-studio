  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-8 px-2">
        <div className={`w-1 h-8 rounded-full ${col.color} shadow-[0_0_15px_var(--column-glow-color)]`} />
...
const CRMBoard: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    const totalPipelineValue = leads.reduce((sum, lead) => {
      const vehicle = ctx.inventory.find(v => v.id === lead.carId || v.name === lead.vehicleOfInterest);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeContainer && overContainer && activeContainer !== overContainer) {
      ...
      setLeads(prev => prev.map(l => l.id === activeLeadId ? { ...l, status: newStatus as any } : l));
    }
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['ID', 'Nombre', 'Email', 'Teléfono', 'Estado', 'Tipo', 'Vehículo Interés', 'AI Score', 'Fecha'];
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-6 pl-4 border-l border-white/5 h-8">
            <div className="flex flex-col">
              <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest">Pipeline</span>
              <span className="text-xs font-black text-emerald-400">${(metrics.totalPipelineValue / 1000).toFixed(1)}k</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest">Leads</span>
        ...
        <div className="hidden lg:flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-500">
          <Filter size={14} />
          Segmentación Activa: <span className="text-white">{filterType === 'finance' ? 'Credit Applications' : filterType === 'contact' ? 'General Leads' : 'Full Pipeline'}</span>
        </div>
      </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 px-1">
        <div className="glass-premium rounded-3xl p-8 border border-white/5 flex flex-col gap-4 group hover:border-cyan-500/30 transition-all">
          ...
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Pipeline Health</div>
        ...
        <div className="glass-premium rounded-3xl p-8 border border-white/5 flex flex-col gap-4 group hover:border-emerald-500/30 transition-all">
          ...
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Potential Revenue</div>
        ...
        <div className="glass-premium rounded-3xl p-8 border border-white/5 flex flex-col gap-4 group hover:border-indigo-500/30 transition-all">
          ...
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Active Conversions</div>
        ...
        <div className="glass-premium rounded-3xl p-8 border border-white/5 flex flex-col gap-4 group hover:border-amber-500/30 transition-all">
          ...
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Escalation Status</div>
  ...
  const renderToolsTab = () => (
    <div className="max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="glass-premium rounded-3xl p-8 border border-white/5">
        <h3 className="text-lg font-black text-white mb-6 uppercase tracking-tight">Data Management</h3>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                ...
                <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">CSV Format • Cloud Secure</div>
              </div>
            </div>
            <LayoutGrid size={16} className="text-slate-600 group-hover:text-white transition-colors" />
          </button>
        </div>
      </div>
    </div>
