...
import { Calculator, Save, Target, DollarSign, TrendingUp, ShieldCheck, FileDown } from 'lucide-react';
import { 
  calculateOTD, 
  calculateLoan, 
  CreditTier, 
...
const DealDesker: React.FC = () => {
  ...
  const handleSaveDeal = async () => {
    try {
      ...
      alert(`✅ Deal #${sessionId.substring(0, 6)} guardado exitosamente en el Workspace de Firebase.`);
    } catch (error) {
      console.error(error);
  ...
  const handleExportPDF = async () => {
    try {
      await generatePDFFromDOM('deal-desker-receipt', `BillOfSale_Richard_${new Date().getTime()}.pdf`);
    } catch (error) {
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-6 relative">
        ...
        <section className="bg-white rounded-4xl p-8 border border-slate-200 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-xl font-black text-slate-800 mb-6 relative z-10 uppercase tracking-widest text-primary">Cuadro 1: Valor del Trato (Frontend)</h2>
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Valor Trade-In</label>
                <div className="relative">
                  <span className="absolute left-4 top-3 text-slate-400 font-bold">$</span>
                  <input type="number" 
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Deuda Trade-In</label>
                <div className="relative">
                  <span className="absolute left-4 top-3 text-slate-400 font-bold">$</span>
                  <input type="number" 
            ...
            <div className="bg-slate-50 p-4 rounded-xl space-y-2 border border-slate-200">
              <div className="flex justify-between text-sm"><span className="text-slate-500">IVU Base</span><span className="font-bold">{formatCurrency(otdCalculation.taxableAmount)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-500">IVU Estatal (10.5%)</span><span className="font-bold text-rose-500">{formatCurrency(otdCalculation.stateTax)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-500">IVU Municipal (1%)</span><span className="font-bold text-rose-500">{formatCurrency(otdCalculation.municipalTax)}</span></div>
              <div className="flex justify-between text-sm pt-2 border-t border-slate-200"><span className="text-slate-500">Doc & Reg. Fees</span><span className="font-bold text-slate-700">{formatCurrency(otdCalculation.dealerFees)}</span></div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-4xl p-8 border border-slate-200 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 left-0 p-8 opacity-5"><ShieldCheck size={150} /></div>
          <h2 className="text-xl font-black text-slate-800 mb-6 relative z-10 uppercase tracking-widest text-primary">Cuadro 2: Productos F&I (Backend)</h2>
          ...
          <div className="space-y-6 relative z-10">
            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors"
                 onClick={() => setGapInsurance(gapInsurance === 0 ? PRODUCTOS_BACKEND_RANGOS.GAP_INSURANCE.default : 0)}>
              <span className="font-bold text-slate-700">Seguro GAP</span>
              <span className={`font-black tracking-widest ${gapInsurance > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                {gapInsurance > 0 ? formatCurrency(gapInsurance) : 'NO APLICA'}
              </span>
            </div>

            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors"
                 onClick={() => setExtendedWarranty(extendedWarranty === 0 ? PRODUCTOS_BACKEND_RANGOS.WARRANTY_EXT.default : 0)}>
              <span className="font-bold text-slate-700">Garantía Extendida (RichCare)</span>
              <span className={`font-black tracking-widest ${extendedWarranty > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                {extendedWarranty > 0 ? formatCurrency(extendedWarranty) : 'NO APLICA'}
              </span>
            </div>
            
            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors"
                 onClick={() => setPaintProtection(paintProtection === 0 ? PRODUCTOS_BACKEND_RANGOS.PAINT_PROTECTION.default : 0)}>
              <span className="font-bold text-slate-700">Protección Cerámica Externa</span>
              <span className={`font-black tracking-widest ${paintProtection > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
            ...
            <div className="pt-4 flex justify-between items-center text-lg">
              ...
              <span className="font-black text-slate-900">{formatCurrency(otdCalculation.totalBackendProducts)}</span>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-4xl p-8 border border-slate-200 shadow-sm col-span-1 lg:col-span-2 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row gap-8 relative z-10">
            ...
            <div className="flex-1 space-y-6">
              <h2 className="text-xl font-black text-slate-800 mb-6 uppercase tracking-widest text-primary">Cuadro 3: Términos y Pronto</h2>
              
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Pronto (Down Payment)</label>
                <input 
                ...
                />
                <div className="text-right font-black text-slate-700 text-lg">{formatCurrency(downPayment)}</div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    {[48, 60, 72, 84].map(t => <option key={t} value={t}>{t} Meses</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Perfil de Crédito</label>
                  <select 
                    title="Perfil de Crédito"
                    value={creditTier} onChange={(e) => setCreditTier(e.target.value as CreditTier)}
                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold"
                  >
            ...
            <div className="w-full lg:w-96 bg-slate-900 rounded-3xl p-8 text-white flex flex-col justify-between shadow-xl">
              <div>
                <h2 className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-6">El Checkpoint final</h2>
                
                <div className="space-y-4">
                  <div className="flex justify-between items-end border-b border-slate-800 pb-4">
                    <span className="text-sm font-medium text-slate-400">Total OTD (Precio Final)</span>
                    <span className="text-2xl font-black text-emerald-400">{formatCurrency(otdCalculation.outTheDoorPrice)}</span>
                  </div>
                  
                  <div className="flex justify-between items-end border-b border-slate-800 pb-4">
                    <span className="text-sm font-medium text-slate-400">Cantidad Financiada (Restando Pronto)</span>
                    <span className="text-xl font-bold">{formatCurrency(loanCalculation.principalFinanced)}</span>
                  </div>

                  <div className="py-2 mt-2">
                    <div className="bg-slate-800 rounded-xl p-4 flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">LTV (Riesgo)</span>
                      <span className={`text-sm font-black ${loanCalculation.ltvRatio > 1.2 ? 'text-rose-500' : 'text-emerald-500'}`}>
              ...
              <div className="mt-8">
                <span className="text-xs uppercase font-bold tracking-widest text-slate-500 block">Pago Mensual</span>
                <div className="flex items-start text-white gap-1 mt-1">
                  <span className="text-2xl font-bold opacity-50 mt-2">$</span>
                  <span className="text-7xl font-black tracking-tighter shadow-primary/50">{loanCalculation.monthlyPayment}</span>
                </div>
