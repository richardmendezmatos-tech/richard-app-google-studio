      ...
      <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
        {gaps.length === 0 ? (
          ...
          gaps.map((gap) => (
            <div
              key={gap.id}
              className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800"
            >
              <div className="flex justify-between items-start mb-1">
                <span className="text-sm font-black text-slate-700 dark:text-slate-200">
                  "{gap.query}"
                </span>
                <span className="text-[10px] font-black text-rose-500 bg-rose-500/10 px-2 py-1 rounded-full uppercase">
                  Perdida
                </span>
              </div>
