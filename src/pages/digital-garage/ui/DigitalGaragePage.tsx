          ...
          return (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 w-full max-w-[1700px]">
              <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                 {[1, 2].map(idx => <SkeletonCarCard key={idx} />)}
              </div>
        ...
        return (
          <div className="space-y-12">
            ...
            <div className="grid gap-4 max-w-4xl">
              ...
              {tradeIns.map((offer) => (
                <div key={offer.id} className="bg-slate-800/50 p-6 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-primary/30 transition-colors">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
        ...
        return (
          <div className="max-w-2xl bg-slate-800/50 p-8 rounded-3xl border border-white/5">
            ...
            <div className="mt-8 p-4 bg-slate-900/50 rounded-xl flex items-center justify-between">
              <div>
                <div className="text-sm font-bold text-white flex items-center gap-2"><ScanFace size={16} className="text-primary" /> Biometría (FaceID)</div>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">{isBiometricEnabled ? 'Activado' : 'Desactivado'}</div>
              </div>
              <button onClick={toggleBiometric} title={isBiometricEnabled ? 'Desactivar FaceID' : 'Activar FaceID'} className={`w-12 h-6 rounded-full p-1 transition-colors ${isBiometricEnabled ? 'bg-primary' : 'bg-slate-700'}`}>
                <div className={`w-4 h-4 rounded-full bg-white transition-transform ${isBiometricEnabled ? 'translate-x-6' : 'translate-x-0'}`} />
              </button>
            </div>
            <button className="w-full mt-8 py-3 bg-red-500/10 text-red-500 font-bold rounded-xl hover:bg-red-500 hover:text-white transition-all">Cerrar Sesión</button>
          </div>
        );
    }
  };
    ...
    <div className="min-h-screen bg-[#0d2232] text-white relative flex flex-col">
      ...
      <header className="p-8 pb-0 relative z-10">
        <div className="flex items-center gap-4 mb-8">
          <button onClick={onExit} title="Regresar a la tienda" className="p-2 hover:bg-white/10 rounded-full transition-colors text-slate-400 hover:text-white">
            <ArrowLeft />
          </button>
          <div>
      ...
      <main className="flex-1 p-8 pt-4 relative z-10 overflow-y-auto custom-scrollbar">
        {isLocked ? (
          <div className="h-full flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in duration-300">
            ...
            <button onClick={handleUnlock} disabled={isScanning} className="px-8 py-4 bg-primary hover:bg-cyan-400 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-cyan-500/30 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed">
              {isScanning ? <><Loader2 className="animate-spin" /> Verificando...</> : <><ScanFace /> Escanear FaceID</>}
            </button>
          </div>
        ) : (
          renderContent()
...
const TabButton = ({ active, onClick, icon, label }: TabButtonProps) => (
  <button onClick={onClick} className={`px-6 py-3 rounded-full font-bold flex items-center gap-2 transition-all ${active ? 'bg-primary text-slate-900 shadow-lg shadow-cyan-500/20' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white'}`}>
    {icon} {label}
  </button>
);
...
const StatusBadge = ({ status }: { status: 'approved' | 'pending' | 'rejected' }) => {
  ...
  const labels: Record<string, string> = { approved: 'Aprobada', pending: 'Pendiente', rejected: 'Rechazada' };
  return <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${styles[status] || styles.pending}`}>{labels[status]}</span>;
};
