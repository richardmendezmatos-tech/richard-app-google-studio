...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [uniqueBrands, inventoryCombinations] = await Promise.all([
    seoService.getUniqueBrands(),
    seoService.getInventoryCombinations(),
  ]);

  // Programmatic SEO: Brand + City Pages (Based on actual inventory)
  ...
  // Programmatic SEO: Model + City Pages (Based on actual inventory)
  const modelCityRoutes: MetadataRoute.Sitemap = SEO_CITIES.flatMap((city) =>
    inventoryCombinations.map((combo) => ({
      url: `${SITE_URL}/autos-usados/${city}/${combo.brand}/${combo.model}`,
      lastModified: new Date(),
      changeFrequency: 'daily' as const,
      priority: 0.85,
    })),
