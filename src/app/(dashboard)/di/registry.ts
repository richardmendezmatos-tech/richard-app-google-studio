...
export const DI = {
  ...
  getLeadsUseCase: () => {
    return {
      execute: async (dealerId: string, limitCount: number = 100) => {
        const { SupabaseLeadRepository } =
          await import('@/entities/lead/api/repositories/SupabaseLeadRepository');
  ...
  getSubscriberRepository: async () => {
    const { SupabaseSubscriberRepository } =
      await import('@/entities/lead/api/repositories/SupabaseCaptureRepository');
    return new SupabaseSubscriberRepository();
  },

  getSurveyRepository: async () => {
    const { SupabaseSurveyRepository } =
      await import('@/entities/lead/api/repositories/SupabaseCaptureRepository');
  ...
  getUserRepository: async () => {
    ...
    try {
      const { SupabaseUserRepository } =
        await import('@/entities/user/api/repositories/SupabaseUserRepository');
  ...
  getAntigravityOutreachAction: (lead: any, context: any) => {
    return import('@/features/omnichannel/api/antigravityOmnichannelService').then((m) =>
      m.getAntigravityOutreachAction(lead, context),
    );
  },

  sendWhatsAppMessage: (to: string, message: string) => {
    return import('@/features/leads/model/whatsappService').then((m) =>
      m.sendWhatsAppMessage(to, message),
    );
  },
  ...
  getIdentifyOutreachOpportunitiesUseCase: () => {
    return {
      execute: async (threshold: number) => {
        const { SupabasePredictiveRepository } =
          await import('@/entities/lead/api/repositories/SupabasePredictiveRepository');
