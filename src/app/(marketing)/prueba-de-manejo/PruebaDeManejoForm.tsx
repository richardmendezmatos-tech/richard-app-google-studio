...
export function PruebaDeManejoForm() {
    ...
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <p className="text-slate-400">
          ...
          <strong className="text-white">{time}</strong> en Vega Alta.
        </p>
        <p className="text-sm text-slate-500">
          Recibirás un recordatorio por WhatsApp. Si necesitas cambiar la cita, llámanos al 787-555-1234.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
        <span className={step === 1 ? 'text-cyan-400' : ''}>Fecha</span>
        <span>/</span>
        <span className={step === 2 ? 'text-cyan-400' : ''}>Datos</span>
      </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="Ej: Ford Explorer 2026"
          ...
          <button
            ...
            onClick={() => setStep(2)}
            disabled={!date || !time}
            className="w-full py-4 bg-cyan-500 hover:bg-cyan-400 text-black font-black text-sm uppercase tracking-widest rounded-xl transition-all disabled:opacity-30"
          >
            Continuar
          </button>
        </div>
