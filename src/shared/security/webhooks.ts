...
export const shouldEnforceWebhookSignatures = (): boolean => {
  const v = String(process.env.WEBHOOK_SIGNATURE_ENFORCE || '')
    .trim()
    .toLowerCase();
...
export const verifySendGridEventWebhook = (req: any, rawBody: Buffer): boolean => {
  ...
  const sigB64 = String(req.headers?.['x-twilio-email-event-webhook-signature'] || '').trim();
  const ts = String(req.headers?.['x-twilio-email-event-webhook-timestamp'] || '').trim();
