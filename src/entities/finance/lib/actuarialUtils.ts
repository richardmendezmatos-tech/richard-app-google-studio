import { Lead, ActuarialReportData, MarketProjection } from '@/shared/types/types';
...
export const generateMockActuarialData = (lead: Lead): ActuarialReportData => {
  ...
  const marketProjections: MarketProjection[] = [
    ...
    { month: 36, estimatedValue: basePrice * 0.6, depreciationPercent: 40 },
  ];

  const riskAnalysis = {
    score: lead.aiScore || 75,
    rating: ((lead.aiScore || 75) > 90 ? 'A+' : (lead.aiScore || 75) > 80 ? 'A' : 'B') as
      | 'A+'
      | 'A'
      | 'B'
      | 'C'
    ...
    observations:
      lead.aiSummary ||
      'El perfil del cliente muestra una intención de compra sólida con estabilidad financiera proyectada. Se recomienda seguimiento prioritario para cierre en el trimestre actual.',
  };
