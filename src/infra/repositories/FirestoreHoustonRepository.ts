...
export class FirestoreHoustonRepository implements HoustonRepository {
    async getTelemetry(): Promise<HoustonTelemetry> {
        ...
        return {
            ...
            lastUpdate: Date.now(),
            metrics: {
                inferenceLatency: { label: 'Inference Latency', value: 240, unit: 'ms', status: 'healthy', trend: 'stable' },
                tokenUsage: { label: 'Token Consumption', value: '1.2M', unit: 'tokens', status: 'healthy', trend: 'up' },
                autonomyRate: { label: 'Autonomy Rate', value: 85, unit: '%', status: 'healthy', trend: 'up' },
                apiStability: { label: 'API Stability', value: 99.9, unit: '%', status: 'healthy', trend: 'stable' }
            },
            recentEvents: [
                { id: '1', timestamp: Date.now(), type: 'info', message: 'Sistema Houston Inicializado', source: 'Core' },
                { id: '2', timestamp: Date.now() - 1000 * 60 * 5, type: 'info', message: 'Sincronización de Inventario Exitosa', source: 'VectorService' }
            ]
        };
    }
