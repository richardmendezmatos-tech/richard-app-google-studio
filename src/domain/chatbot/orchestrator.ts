    ...
    public nextState(
        ...
        if (message.match(/(inventario|stock|modelo|tienes|disponible|color)/)) {
            return {
                ...
                dataCollected: {
                    ...currentState.dataCollected,
                    hasVehicleId: currentState.dataCollected.hasVehicleId || !!currentLead.vehicleOfInterestId
                }
            };
        }

        // 3. Detect Flow Shift - Closing (High Intent)
        if (currentLead.intentScore > 90 || message.match(/(comprar|trato|oferta|cita|hoy mismo|listo)/)) {
            return {
                ...currentState,
