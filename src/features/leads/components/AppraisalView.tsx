    ...
    <div className="appraisal-progress-container h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-6">
      <div
        className={`h-full transition-all duration-500 ${
          count === 4 ? 'bg-emerald-500 progress-filled' : 'bg-[#00aed9] progress-active'
        } ${
          count === 0
            ? 'w-0'
            : count === 1
      ...
      {Object.keys(photos).map((key) => (
        ...
        >
          {photos[key] ? (
            ...
            <>
              <Camera
                className="text-slate-400 mb-2 group-hover:text-[#00aed9] group-hover:scale-110 transition-all"
                size={24}
              />
