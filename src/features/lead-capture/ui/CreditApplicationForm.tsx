...
export const CreditApplicationForm: React.FC = () => {
    ...
    return (
      <div className="bg-slate-900/60 backdrop-blur-2xl border border-emerald-500/20 rounded-4xl p-12 text-center text-white max-w-2xl mx-auto shadow-[0_0_80px_-20px_rgba(16,185,129,0.3)]">
        ...
        <p className="text-slate-300 text-lg mb-8 leading-relaxed font-medium">
          Tus credenciales financieras han sido encriptadas y transferidas directamente a la bóveda privada de nuestro departamento de F&I. <br/><br/>
          Nuestro algoritmo algorítmico y especialistas humanos ya están peleando la mejor tasa del mercado para ti. 🚗💨
        </p>
        <button 
  ...
  const inputClasses = "w-full p-4 bg-black/40 border border-white/10 text-white placeholder-slate-500 rounded-xl font-medium focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-400 focus:bg-black/60 outline-none transition-all";
  ...
  return (
    <div className="relative bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] p-8 md:p-12 shadow-[0_40px_100px_-20px_rgba(0,180,216,0.25)] border border-white/10 max-w-3xl mx-auto overflow-hidden">
      ...
      <div className="relative flex items-center gap-5 mb-8 pb-8 border-b border-white/10 z-10">
        ...
        <div>
          <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter">Bóveda de Pre-Cualificación</h2>
      ...
      <div className="relative flex items-center gap-4 mb-10 z-10">
        {[
          ...
          <div key={s.id} className="flex-1">
            ...
            <p className={`text-xs md:text-sm font-bold mt-1 ${step >= s.id ? 'text-white' : 'text-slate-500'}`}>
              {s.label}
            </p>
          </div>
        ))}
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div className="bg-cyan-500/10 border border-cyan-500/20 p-4 rounded-xl mb-6 flex gap-4 items-start">
                <div className="text-cyan-400 mt-1"><ShieldCheck size={20} /></div>
                <p className="text-sm font-medium text-slate-300">
                  Protegemos tu identidad civil rigurosamente. Usa tu nombre legal exactamente como aparece en tu Licencia de Conducir.
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className={labelClasses}>Nombre Legal</label>
                  <input required name="firstName" value={formData.firstName} onChange={handleChange} autoComplete="given-name" className={inputClasses} placeholder="Ej. Juan D." />
                </div>
                <div>
                  <label className={labelClasses}>Apellidos</label>
                  <input required name="lastName" value={formData.lastName} onChange={handleChange} autoComplete="family-name" className={inputClasses} placeholder="Ej. Pérez Cintrón" />
                </div>
                <div>
                  <label className={labelClasses}>Correo Electrónico Activo</label>
                  <input required type="email" name="email" value={formData.email} onChange={handleChange} autoComplete="email" inputMode="email" className={inputClasses} placeholder="Para enviarte la aprobación" />
                </div>
                <div>
                  <label className={labelClasses}>Teléfono Celular (Acepta SMS)</label>
                  <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} autoComplete="tel" inputMode="tel" className={inputClasses} placeholder="(787) 555-0000" />
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  <input required name="addressLine1" value={formData.addressLine1} onChange={handleChange} autoComplete="street-address" className={inputClasses} placeholder="Urb., Condominio o Calle" />
                </div>
                <div>
                  <label className={labelClasses}>Pueblo / Municipio</label>
                  <input required name="city" value={formData.city} onChange={handleChange} autoComplete="address-level2" className={inputClasses} placeholder="Ej. San Juan" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required type="number" name="timeAtAddress" value={formData.timeAtAddress} onChange={handleChange} inputMode="numeric" className={inputClasses} placeholder="Ej. 3" min="0" />
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  <input required name="employer" value={formData.employer} onChange={handleChange} autoComplete="organization" className={inputClasses} placeholder="Nombre de la compañía o 'Cuenta Propia'" />
                </div>
                <div>
                  <label className={labelClasses}>Título Institucional / Cargo</label>
                  <input required name="jobTitle" value={formData.jobTitle} onChange={handleChange} autoComplete="organization-title" className={inputClasses} placeholder="Ej. Analista, Dueño..." />
                </div>
                <div>
                  <label className={labelClasses}>Estabilidad (Años Laborando)</label>
                  <input required type="number" name="timeAtEmployer" value={formData.timeAtEmployer} onChange={handleChange} inputMode="numeric" className={inputClasses} placeholder="0 si es reciente" />
                </div>
                <div className="md:col-span-2 mt-2">
                  ...
                  <div className="relative">
                    <span className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-400 font-bold text-xl">$</span>
                    <input required type="number" name="monthlyIncome" value={formData.monthlyIncome} onChange={handleChange} inputMode="numeric" className="w-full pl-10 pr-4 py-5 bg-black/60 border border-emerald-500/30 text-emerald-400 rounded-xl font-black text-2xl outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-400 shadow-[inset_0_2px_15px_rgba(0,0,0,0.5)] transition-all" placeholder="0.00" />
                  </div>
                  <p className="text-[11px] text-slate-500 mt-2 font-medium">Incluye horas extras, bonos, y otros ingresos demostrables antes de deducciones.</p>
                </div>
                
                <div className="md:col-span-2 mt-4 pt-6 border-t border-white/10">
                  <div className="flex items-center gap-2 mb-3">
                    <ShieldCheck size={18} className="text-cyan-400" />
                    <label className="text-sm font-black text-white uppercase tracking-widest">Seguro Social Completo (Obligatorio)</label>
                  </div>
                  <input required type="tel" name="ssn" placeholder="XXX-XX-XXXX (9 Dígitos)" value={formData.ssn} onChange={handleChange} className="w-full p-5 bg-slate-950/80 border border-slate-700 rounded-xl font-bold focus:ring-2 focus:ring-cyan-500/40 outline-none transition-all text-2xl tracking-[0.2em] text-cyan-100 font-mono shadow-[inset_0_2px_20px_rgba(0,0,0,0.8)]" />
                  <p className="text-[11px] text-slate-500 mt-3 font-medium flex items-center gap-1.5 leading-tight">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0"></span>
                    Para procesar su aprobación bancaria, requerimos su Seguro Social de forma íntegra. Richard Automotive transmite estos datos por un túnel cifrado de 256-Bit SSL directo al buró para garantizar su seguridad legal total.
                  </p>
                </div>
              </div>
            </motion.div>
