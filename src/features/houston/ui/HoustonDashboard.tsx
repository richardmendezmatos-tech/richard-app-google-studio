...
export const HoustonDashboard: React.FC = () => {
  ...
  useLayoutEffect(() => {
    if (healthRef.current && telemetry) {
      ...
      const healthValue =
        typeof telemetry.metrics.structuralHealth.value === 'string'
    ...
    <div className="p-8 bg-slate-950/40 backdrop-blur-xl border border-cyan-500/20 rounded-4xl shadow-2xl shadow-cyan-500/5 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-8 relative">
        ...
        <div className="hidden sm:flex items-center gap-3 px-5 py-2.5 bg-slate-950/50 rounded-2xl border border-white/5 shadow-2xl backdrop-blur-3xl">
          <div
            className={`w-2 h-2 rounded-full ${telemetry.systemHealth === 'online' ? 'bg-emerald-500' : 'bg-amber-500'} animate-ping shadow-[0_0_10px_rgba(16,185,129,0.5)]`}
          />
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none">
            RA SYNC:{' '}
            <span
              className={
                telemetry.systemHealth === 'online' ? 'text-emerald-500' : 'text-amber-500'
              }
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-premium p-4 border border-white/5">
              ...
              <p className="text-2xl font-black text-white">
                {businessData?.summary.leads_last_24h || 0}
              </p>
            </div>
            <div className="glass-premium p-4 border border-white/5">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">
              ...
              <p className="text-2xl font-black text-primary">
                {businessData?.summary.avg_score || 0}%
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex justify-between text-[8px] font-black text-slate-500 uppercase tracking-widest mb-2">
                  ...
                  <span
                    className={
                      telemetry.metrics.lcp.status === 'healthy'
                        ? 'text-emerald-400'
                        : 'text-amber-400'
                    }
                  >
                    {telemetry.metrics.lcp.value}
                    {telemetry.metrics.lcp.unit || 'ms'}
                  </span>
                </div>
                <div className="h-0.5 bg-slate-900 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (2500 / (Number(telemetry.metrics.lcp.value) || 2500)) * 100)}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="glass-premium p-4 border border-indigo-500/10 flex items-center justify-between group hover:border-indigo-500/20 transition-all">
            <div>
              ...
              <div className="text-xl font-black text-white">
                {telemetry.metrics.inferenceLatency.value}
                {telemetry.metrics.inferenceLatency.unit}
              </div>
            </div>
          ...
          <div className="glass-premium p-4 border border-white/5">
            ...
            <div className="flex justify-between">
              {['Inv', 'Lead', 'Fin'].map((label) => (
                <div
                  key={label}
                  className="w-10 h-10 rounded-full border-2 border-slate-900 flex items-center justify-center relative"
                >
                  <div
