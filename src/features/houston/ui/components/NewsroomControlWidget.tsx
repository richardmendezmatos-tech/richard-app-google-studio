    ...
    <div className="glass-premium p-6 border border-white/5 group hover:border-cyan-500/20 transition-all overflow-hidden relative hud-brackets">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.4em]">
              Editorial Control
            </h4>
            <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">
              Gestión de Contenido Sentinel
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          {isLoading ? (
            ...
            posts.map((post) => (
              <div
                key={post.id}
                className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between group/item hover:bg-white/[0.04] transition-all"
              >
