  ...
  useEffect(() => {
    if (connectionState === 'connected') {
      const interval = setInterval(() => {
        ...
        getAuditRepository()
          .then((repo) => repo.getRecentLogs(10))
          .then((logs) => {
            setTelemetry((prev) => ({
              ...prev,
              lastUpdate: Date.now(),
              ...
              metrics: {
                ...
                closureProbability: {
                  ...prev.metrics.closureProbability,
                  value: closureProbability,
                },
                dbLatency: { ...prev.metrics.dbLatency, value: dbLatency },
                inferenceLatency: {
                  ...prev.metrics.inferenceLatency,
                  value: Math.round(inferenceLatency),
                },
                structuralHealth: {
                  ...prev.metrics.structuralHealth,
                  value: Math.round(structuralHealth),
                },
                lcp: {
                  ...prev.metrics.lcp,
                  value: Math.round(lcp),
                  status: lcp < 1200 ? 'healthy' : 'warning',
                },
                fid: {
                  ...prev.metrics.fid,
                  value: Math.round(fid),
                  status: fid < 100 ? 'healthy' : 'warning',
                },
                cls: {
                  ...prev.metrics.cls,
                  value: cls.toFixed(4),
                  status: cls < 0.1 ? 'healthy' : 'warning',
                },
              },
            }));
          });
