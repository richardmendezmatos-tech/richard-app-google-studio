...
const ComparisonView: React.FC = () => {
  ...
  // Track comparison when 2 or more cars are present
  useEffect(() => {
    if (selectedCars.length >= 2) {
      analytics.trackCompare(selectedCars[0].id, selectedCars[1].id);
    }
  }, [selectedCars, analytics]);
  ...
  if (selectedCars.length === 0) {
    return (
      <div className="min-h-screen bg-[#0d2232] flex flex-col items-center justify-center p-8 text-center text-white">
        ...
        <button onClick={() => navigate('/')} className="text-[#00aed9] font-bold hover:underline">
          Ir a la Tienda
        </button>
      </div>
    );
      ...
      <header className="p-8 relative z-10 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#00aed9] border border-[#00aed9]/30 rounded-lg hover:bg-[#00aed9]/10"
          >
            Limpiar Todo
          </button>
        </div>
      ...
      <main className="p-8 relative z-10 max-w-[1800px] mx-auto pb-24">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter mb-4">
            Batalla de <span className="text-[#00aed9]">Especificaciones</span>
          </h1>
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 border border-[#00aed9]/30 rounded-3xl p-8 mb-12 relative overflow-hidden shadow-2xl shadow-cyan-900/20">
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
              <div className="shrink-0 flex flex-col items-center">
                ...
                <span className="text-[10px] uppercase font-black tracking-widest text-[#00aed9]">
                  Richard AI Veredict
                </span>
              </div>
              <div className="text-center md:text-left">
                <h3 className="text-2xl font-bold text-white mb-2">
                  El Ganador es:{' '}
                  <span className="text-[#00aed9]">
                    {selectedCars.find((c) => c.id === verdict.winnerId)?.name}
                  </span>
                </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          {selectedCars.map((car) => {
            ...
            return (
              <div
                key={car.id}
                className={`relative flex flex-col ${isWinner ? 'scale-105 z-10' : 'opacity-90 grayscale-[0.3]'}`}
              >
                {isWinner && (
                ...
                {/* Car Header */}
                <div
                  className={`bg-slate-800 rounded-t-3xl p-6 relative overflow-hidden ${isWinner ? 'border-2 border-[#00aed9]' : 'border border-white/5'}`}
                >
                  <button
                  ...
                  <h3 className="text-xl font-black text-white leading-tight mb-1 min-h-[50px]">
                    {car.name}
                  </h3>
                ...
                {/* Specs */}
                <div
                  className={`bg-slate-900/50 rounded-b-3xl p-6 space-y-4 border-x border-b ${isWinner ? 'border-[#00aed9] bg-[#00aed9]/5' : 'border-white/5'}`}
                >
                  ...
                  <div className="h-12 flex items-center justify-between lg:justify-center border-b border-white/5">
                    ...
                    <span className="text-[#00aed9] font-bold">
                      ${Math.round((car.price || 0) / 72).toLocaleString()}
                    </span>
                  </div>
                  <div className="h-12 flex items-center justify-between lg:justify-center border-b border-white/5">
                  ...
                  <div className="h-12 flex items-center justify-between lg:justify-center border-b border-white/5">
                    ...
                    <div className="flex gap-1 justify-center">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <div
                          key={i}
                          className={`w-2 h-2 rounded-full ${i <= (car.name.includes('Hyundai') ? 5 : 4) ? 'bg-[#00aed9]' : 'bg-slate-700'}`}
                        />
                      ))}
                    </div>
                  </div>
