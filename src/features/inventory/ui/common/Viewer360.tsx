...
const Viewer360: React.FC<Props> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`aspect-[4/3] bg-linear-to-br from-slate-100 to-white dark:from-slate-900 dark:to-slate-950 rounded-4xl shadow-2xl relative overflow-hidden group select-none cursor-grab active:cursor-grabbing border border-white/10 transition-all duration-700 ${isScanning ? 'ring-4 ring-cyan-500/50 scale-[0.98]' : ''}`}
      onMouseDown={(e) => handleStart(e.clientX)}
      onMouseMove={(e) => (activeMode === '360' ? handleMove(e.clientX) : handleCinematicMove(e))}
      onMouseUp={handleEnd}
      onMouseLeave={() => {
        handleEnd();
        handleCinematicLeave();
      }}
    ...
    >
      ...
      {/* AI Callouts */}
      <div
        className={`absolute inset-0 z-30 pointer-events-none transition-opacity duration-500 ${showCallouts ? 'opacity-100' : 'opacity-0'}`}
      >
        <div className="absolute top-[20%] left-[20%] animate-in zoom-in slide-in-from-left-4 duration-700">
          <div className="bg-slate-900/90 backdrop-blur border border-cyan-500/30 p-3 rounded-2xl shadow-xl flex items-center gap-3">
            ...
            <div>
              <p className="text-[8px] text-cyan-400 font-bold uppercase tracking-tight">
                Potencia
              </p>
      ...
      {/* Hints Overlay */}
      <div
        className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-opacity duration-700 ${hintVisible ? 'opacity-100' : 'opacity-0'}`}
      >
