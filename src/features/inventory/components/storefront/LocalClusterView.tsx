...
const LocalClusterView: React.FC<LocalClusterViewProps> = ({ inventory }) => {
    ...
    const normalizedLocation = location ? location.charAt(0).toUpperCase() + location.slice(1).toLowerCase() : '';
    ...
    return (
        <div className="min-h-screen bg-[#020810]">
            ...
            <HeroSection
                ...
                onBrowseInventory={() => document.getElementById('cluster-grid')?.scrollIntoView({ behavior: 'smooth' })}
            ...
            <main className="relative z-20 mx-auto -mt-14 max-w-[1600px] space-y-14 px-5 pb-28 lg:px-12">
                <section id="cluster-grid" className="scroll-mt-32">
                    ...
                    <div className="mb-10 flex flex-col gap-4 px-2">
                        <div className="flex items-center gap-2 text-cyan-400">
                            <MapPin size={18} />
                            <span className="font-tech text-xs uppercase tracking-[0.2em]">Puerto Rico / {normalizedLocation}</span>
                        </div>
                    ...
                    {filteredCars.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center rounded-[40px] border-2 border-dashed border-cyan-300/20 bg-[#081726]/85 py-24 text-center">
                            ...
                            <h3 className="font-cinematic text-3xl text-white">No hay {normalizedCategory} disponibles ahora</h3>
                            <p className="mt-2 text-slate-400">Estamos renovando nuestro inventario en {normalizedLocation} constantemente.</p>
                            <button
                                onClick={() => navigate('/')}
                ...
                <section className="rounded-[30px] border border-white/5 bg-white/5 p-8 text-center backdrop-blur-sm">
                    <div className="mx-auto flex max-w-xl flex-col items-center gap-4">
                        ...
                        <h3 className="text-xl font-bold text-white">¿No encuentras lo que buscas en {normalizedLocation}?</h3>
                        <p className="text-slate-400">
                            Nuestra IA está rastreando nuevas unidades 24/7. Richard Automotive ofrece entrega a domicilio en todo Puerto Rico.
                        </p>
                    </div>
                </section>
            </main>
