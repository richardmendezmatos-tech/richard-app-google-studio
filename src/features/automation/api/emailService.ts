...
const sendTemplateOrFallback = async ({
  ...
  if (resolvedTemplateId) {
    ...
    const { sendTemplateEmail } = await import('@/shared/api/communications/SendGridAdapter');
  ...
  console.warn('Template ID missing, sending fallback plain email', { to, subject });
  return notifier.execute({
    to,
    subject,
    html,
...
export const sendWelcomeEmail1 = async (lead: LeadEmailData) => {
  if (!lead.email) {
    console.warn('No email provided for lead:', lead.nombre);
    return;
  }
