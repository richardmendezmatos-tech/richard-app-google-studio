...
class AutomationService {
  private async dispatchOutreach(
    lead: Lead,
    message: string,
    subject?: string,
  ): Promise<{ success: boolean; channel: 'whatsapp' | 'email' }> {
    const antigravityAction = await DI.getAntigravityOutreachAction(lead, {
      trigger: 'nurture',
    ...
    if (lead.phone) {
      const { success: sent } = await DI.sendWhatsAppMessage(lead.phone, finalMessage);
  ...
  async processLeadNurturing(lead: Lead): Promise<void> {
    ...
    if (
      rawCreatedAt &&
      typeof (rawCreatedAt as { toDate: () => { getTime: () => number } }).toDate === 'function'
    ) {
      createdAtTime = (rawCreatedAt as { toDate: () => { getTime: () => number } })
        .toDate()
        .getTime();
