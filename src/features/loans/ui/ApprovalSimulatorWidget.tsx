...
const PROBABILITY_BY_TIER: Record<
  string,
  { percent: number; color: string; label: string; glow: string }
> = {
  excellent: {
    percent: 98,
...
export const ApprovalSimulatorWidget: React.FC<ApprovalSimulatorWidgetProps> = ({
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto overflow-hidden rounded-5xl bg-slate-950/40 backdrop-blur-3xl border border-white/5 shadow-[0_48px_96px_-24px_rgba(0,0,0,0.8)] p-0.5 group/sim">
      ...
      <div
        className={`absolute -top-32 -right-32 w-80 h-80 blur-[120px] opacity-20 rounded-full transition-all duration-1000 ${currentProb.glow}`}
      ></div>
      <div
        className={`absolute -bottom-32 -left-32 w-80 h-80 blur-[120px] opacity-10 rounded-full transition-all duration-1000 ${currentProb.glow}`}
      ></div>
        ...
        <div className="flex items-start justify-between mb-12">
          <div className="flex items-center space-x-5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 bg-ra-primary/20 border border-ra-primary/30 rounded text-[9px] font-black text-ra-primary uppercase tracking-widest">
                  Ultra
                </span>
              </div>
              <p className="text-[10px] text-white/40 uppercase tracking-[0.4em] font-bold mt-1">
                Intelligence Layer v4.2
              </p>
            </div>
          </div>
          ...
          <div className="hidden sm:flex flex-col items-end">
            <div className="flex items-center space-x-3 px-4 py-2 bg-white/[0.03] rounded-full border border-white/5 backdrop-blur-md">
              <div
                className={`w-2.5 h-2.5 rounded-full animate-ping ${currentProb.glow.replace('bg', 'bg')}`}
              ></div>
              <div className={`absolute w-2.5 h-2.5 rounded-full ${currentProb.glow}`}></div>
              <span
                className={`text-[10px] font-black uppercase tracking-[0.2em] ${currentProb.color}`}
              >
                {currentProb.label}
              </span>
            </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="relative group/input">
                    ...
                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within/input:text-ra-primary transition-colors font-black">
                      $
                    </span>
                    <input
                      type="number"
                  ...
                  {!showTradeIn ? (
                    ...
                    >
                      <div className="flex justify-between items-center mb-5">
                        ...
                        <span className="text-2xl font-black text-ra-accent tracking-tighter">
                          ${tradeIn.toLocaleString()}
                        </span>
                      </div>
                      <input
                        type="range"
              ...
              {/* Credit Tier Matrix */}
              <div>
                <label className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] ml-2 mb-4 block">
                  Estatus de Crédito
                </label>
              ...
              <div className="pt-10 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-10">
                <div className="text-center md:text-left">
                  <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] mb-3">
                    Inversión Mensual Proyectada
                  </p>
                  <div className="flex items-baseline justify-center md:justify-start space-x-3">
                    ...
                    <span className="text-ra-primary font-black text-xs tracking-[0.3em] uppercase">
                      / mes*
                    </span>
                  </div>
                </div>
          ...
          {step === 3 && (
            ...
            >
              ...
              <p className="text-white/40 leading-relaxed mb-12 max-w-sm mx-auto text-sm font-bold uppercase tracking-wider">
                ...
                <strong className="text-ra-primary">${Math.round(monthlyPayment)}/mes</strong>.
                Jules ha notificado a nuestro equipo VIP para tu atención inmediata.
              </p>
