          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-[11px] font-bold text-white/30 uppercase tracking-widest mb-3 block">Precio de la Unidad</label>
                  <div className="relative group">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-cyan-400 transition-colors">$</span>
                    <input 
                      type="number"
                      value={price}
                      onChange={(e) => setPrice(Number(e.target.value))}
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-9 pr-4 text-white hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all font-bold text-lg"
                    />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] font-bold text-white/30 uppercase tracking-widest mb-3 block">Término (Meses)</label>
                  <select 
                    value={term}
                    onChange={(e) => setTerm(Number(e.target.value))}
                    className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-4 text-white hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all font-bold text-lg appearance-none"
                  >
                ...
                <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 space-y-6">
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <span className="text-lg font-black text-white">${downPayment.toLocaleString()}</span>
                    </div>
                    <input 
                  ...
                  {!showTradeIn ? (
                    <button 
                      ...
                      className="flex items-center space-x-2 text-xs font-bold text-cyan-400/70 hover:text-cyan-400 transition-colors"
                    >
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="pt-4 border-t border-white/5">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-lg font-black text-amber-400">${tradeIn.toLocaleString()}</span>
                      </div>
                      <input 
              ...
              <div>
                <label className="text-[11px] font-bold text-white/30 uppercase tracking-widest mb-4 block">Perfil de Crédito</label>
                <div className="grid grid-cols-2 gap-3">
                  {(Object.keys(CREDIT_RATES) as CreditTier[]).map((t) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setTier(t)}
                      className={`relative py-4 px-4 rounded-2xl border text-sm font-bold transition-all overflow-hidden group ${
                        tier === t 
                        ? 'bg-cyan-500/10 border-cyan-400/50 text-white shadow-[0_0_20px_rgba(34,211,238,0.1)]' 
                        : 'bg-white/5 border-white/5 text-white/40 hover:bg-white/10 hover:border-white/10'
                      }`}
                    >
                      {tier === t && <motion.div layoutId="active-tier" className="absolute inset-0 bg-cyan-400/5 z-0" />}
              ...
              <div className="mt-10 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="text-center md:text-left">
                  <p className="text-[10px] text-white/30 mb-2 uppercase tracking-[0.2em] font-black">Tu Inversión Mensual</p>
                  <div className="flex items-baseline justify-center md:justify-start space-x-2">
                    <span className="text-5xl font-black text-white tracking-tighter">${Math.round(monthlyPayment)}</span>
                    <span className="text-white/30 font-bold text-sm tracking-widest">/MES</span>
                  </div>
                </div>
                <button
                  onClick={() => setStep(2)}
                  className="w-full md:w-auto px-10 py-5 bg-white text-black hover:bg-cyan-400 transition-all rounded-full font-black text-sm uppercase tracking-widest shadow-[0_20px_40px_-12px_rgba(255,255,255,0.2)] flex items-center justify-center space-x-3 group"
                >
          ...
          {step === 2 && (
            ...
            >
              ...
              <div className="pt-6 flex space-x-4">
                ...
                <button type="submit" disabled={isLoading} className="flex-1 py-4 bg-cyan-400 text-black rounded-full font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-white transition-all flex justify-center items-center">
  ...
  return (
    <div className="space-y-2">
      ...
      <input 
        ...
        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-5 text-white text-sm focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all hover:bg-white/10 placeholder:text-white/10" 
      />
    </div>
  );
}
