          ...
          {!showDetails ? (
            <>
              <p className="mb-6 text-sm leading-relaxed text-slate-300">
                Utilizamos tecnologias de IA y analisis para mejorar tu experiencia. Tu tienes el
...
const PrivacyToggle = ({
  ...
  <div
    ...
    className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 ${
      enabled ? 'bg-cyan-500/5 border-cyan-500/30' : 'bg-slate-800/50 border-white/5'
    }`}
  >
    <div
      className={`p-2 rounded-lg ${enabled ? 'bg-cyan-500 text-white' : 'bg-slate-700 text-slate-500'}`}
    >
      <Icon size={16} />
    </div>
    ...
    <div
      className={`w-10 h-5 rounded-full relative transition-colors ${enabled ? 'bg-cyan-500' : 'bg-slate-700'}`}
    >
      <div
