    ...
    <div className="bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-3xl p-6 space-y-4 hover:border-emerald-500/20 transition-all group">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center border ${status === 'connected' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}
          >
            <MessageSquare
              size={20}
              className={status === 'connected' ? 'text-emerald-400' : 'text-rose-400'}
            />
          </div>
          <div>
      ...
      {status === 'disconnected' && (
        <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl space-y-2">
          ...
          <p className="text-[10px] text-slate-400 leading-tight">
            Para activar la IA en WhatsApp, vincula tu{' '}
            <span className="text-white font-bold">Access Token</span> en la configuración.
          </p>
