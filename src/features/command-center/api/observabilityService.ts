...
export class ObservabilityService {
  static async logAIUsage(metrics: AIUsageMetrics): Promise<void> {
    ...
    console.log(
      `AI Usage: ${metrics.feature} | ${metrics.model} | ${totalTokens} tokens | ${metrics.latencyMs}ms`,
    );
  ...
  static async triggerCriticalAlert(title: string, message: string, source: string): Promise<void> {
    ...
    try {
      const { sendNotificationEmail } = await import('@/features/automation/api/emailService');
  ...
  static async trackOperation<T>(
    ...
    operation: () => Promise<{ tokens: { prompt: number; completion: number }; result: T }>,
  ): Promise<T> {
