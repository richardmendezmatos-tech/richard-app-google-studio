...
import {
  recibirNuevaUnidad,
  RecibirUnidadParams,
} from '@/features/inventory/application/RecibirNuevaUnidad';
import { useNotification } from '@/contexts/NotificationContext';

const AdminInTakeView: React.FC = () => {
        ...
        <header className="relative z-10 mb-12 flex justify-between items-end">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-gradient-to-br from-[#00aed9] to-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-cyan-500/20">
              ...
              <div key={s} className="flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black border transition-all ${
                    step >= s
                      ? 'bg-[#00aed9] border-[#00aed9] text-white'
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => setStep(2)}
                  className="h-14 px-10 bg-white text-black font-black uppercase text-xs tracking-widest rounded-2xl flex items-center gap-3 hover:bg-[#00aed9] hover:text-white transition-all group"
                >
          ...
          {step === 2 && (
            ...
            >
              <div className="bg-white/5 border border-white/5 rounded-[40px] p-10 mb-8">
                ...
                <div className="mt-10 p-6 bg-[#00aed9]/5 border border-[#00aed9]/20 rounded-3xl flex items-start gap-4">
                  <ShieldCheck className="text-[#00aed9] shrink-0" size={20} />
                  <div>
                    <h4 className="text-[10px] font-black text-[#00aed9] uppercase tracking-widest mb-1">
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="h-16 px-12 bg-[#00aed9] text-white font-black uppercase text-xs tracking-[0.2em] rounded-2xl flex items-center gap-3 shadow-2xl shadow-cyan-500/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale"
                >
          ...
          {step === 3 && (
            <div className="animate-in zoom-in-95 fade-in duration-700 flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-slate-400 max-w-md mx-auto mb-12">
                La unidad con VIN <span className="text-[#00aed9] font-mono">{formData.vin}</span>{' '}
...
const InputGroup: React.FC<{
  ...
  <div className="group">
    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block ml-1 group-focus-within:text-[#00aed9] transition-colors">
      {label}
    </label>
    <div className="relative">
      <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#00aed9] transition-colors">
        <Icon size={18} />
      </div>
      <input
        type={type}
        ...
        className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl pl-12 pr-4 text-white font-bold transition-all focus:bg-white/10 focus:border-[#00aed9]/50 focus:ring-0 outline-none"
      />
    </div>
  </div>
);

export default AdminInTakeView;
