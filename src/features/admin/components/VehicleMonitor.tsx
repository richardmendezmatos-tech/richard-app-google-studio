    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 gap-4">
        ...
        <p className="font-bold text-slate-500 animate-pulse">
          Sincronizando con Richard IoT Bridge...
        </p>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <div
              className={`w-2 h-2 rounded-full ${telemetry ? 'bg-green-500 animate-pulse' : 'bg-slate-400'}`}
            />
            <span className="text-[10px] font-black uppercase text-slate-400">
              {telemetry ? 'Live Data Active' : 'Disconnected'}
            </span>
          </div>
        </div>
      ...
      {!telemetry && !simulator && (
        <div className="text-center py-24 bg-white dark:bg-slate-800/80 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700 route-fade-in">
          ...
          <p className="text-slate-400 max-w-md mx-auto px-6">
            There is no active telemetry data for this unit. Start the simulator above or verify the
            hardware dongle connection.
          </p>
        </div>
      )}
          ...
          <div className="col-span-1 md:col-span-2 lg:col-span-4 bg-white dark:bg-slate-800/80 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-xl ${health?.overallStatus === 'critical' ? 'bg-red-500' : health?.overallStatus === 'warning' ? 'bg-amber-500' : 'bg-emerald-500'}`}
                >
              ...
              {health?.alerts && health.alerts.length > 0 ? (
                ...
                <div className="col-span-full py-8 text-center bg-emerald-500/5 border border-dashed border-emerald-500/20 rounded-2xl">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-1">
                    No se detectan anomalías predictivas en los componentes core.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex flex-col justify-center gap-4">
                ...
                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                  ...
                  <p className="text-xl font-black text-emerald-700 dark:text-emerald-300">
                    ...
                    <span className="text-xs font-bold text-emerald-500 opacity-60">Stable</span>
                  </p>
                </div>
              </div>
            </div>
