...
export const MarketingTabView: React.FC<MarketingTabViewProps> = ({ inventory, subscribers, setMarketingCar }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-[600px]">
      <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-xl rounded-[2rem] p-8 border border-white/10 shadow-2xl space-y-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#00aed9]/10 rounded-full blur-[80px] pointer-events-none" />
        
        <div className="flex items-center gap-3 text-[#00aed9] font-black text-xs uppercase tracking-[0.2em] relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
          {inventory.slice(0, 4).map((car) => (
            <button
              ...
              className="flex items-center gap-4 p-4 bg-slate-800/40 rounded-2xl border border-white/5 hover:border-[#00aed9]/50 hover:bg-slate-800/80 hover:shadow-[0_0_20px_rgba(0,174,217,0.15)] hover:-translate-y-1 transition-all duration-300 text-left group"
            >
              <img
              ...
              />
              <div>
                <div className="text-sm font-black text-white uppercase tracking-tight group-hover:text-[#00aed9] transition-colors">
                  {car.name}
                </div>
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                  Planear Post <span className="text-[#00aed9]">✨</span>
                </div>
              </div>
            </button>
          ))}
      ...
      <div className="lg:col-span-1 h-full space-y-6">
        <Suspense fallback={<div className="h-48 rounded-[2rem] bg-slate-900/40 animate-pulse border border-white/5" />}>
          <GapAnalyticsWidget />
        </Suspense>
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-[2rem] p-6 border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[400px] relative">
          ...
          <div className="flex justify-between items-center mb-6 relative z-10">
            ...
            <span className="text-3xl font-black text-white text-glow glow-emerald-500/50">{subscribers.length}</span>
          </div>
          
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 relative z-10 pr-2">
            {subscribers.map((sub: any, i: number) => (
              <div key={sub.id || i} className="p-3 bg-slate-800/30 rounded-xl border border-white/5 flex flex-col shadow-inner hover:bg-slate-800/60 hover:border-white/10 transition-colors">
