...
export class FirestoreLeadRepository implements LeadRepository {
  ...
  async getLeadsByVehicleId(vehicleId: string): Promise<Lead[]> {
    const snapshot = await db.collection('applications').where('vehicleId', '==', vehicleId).get();
  ...
  async getLeadsByEmailSequenceStatus(
    ...
    const safeOperator = (
      validOperators.includes(operator) ? operator : '=='
    ) as admin.firestore.WhereFilterOp;
