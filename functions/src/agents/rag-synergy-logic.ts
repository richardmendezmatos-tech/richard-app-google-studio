...
export async function orchestrateResponse(input: {
    ...
    const marketIntelPromise = input.vehicleContext?.make && input.vehicleContext?.model
    ...
    try {
        ...
        if (!auditData.passed && auditData.correctedResponse) {
            ...
            logger.warn('Validation FAILED — Logic Checksum triggered corection', { feedback: auditData.feedback });
        }
    } catch (e) {
    ...
    const negotiationPromise = (async () => {
        ...
        if (lowerMsg.includes('pago') || lowerMsg.includes('mensual') || lowerMsg.includes('pronto') || lowerMsg.includes('caro')) {
