...
import { MessageSquare, Code, ImageIcon, Settings, FlaskConical, Clapperboard, Mic, ArrowLeft, Github } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, onExit }) => {
  const navItems = [
    ...
    { id: ViewMode.VOICE_ASSISTANT, label: 'Voice Core', icon: <Mic size={20} /> },
        ...
        <div className="px-3 space-y-2">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-4 rounded-xl transition-all duration-200 group relative ${currentView === item.id
