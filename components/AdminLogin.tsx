    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-900 text-white">
      ...
      <div className="w-full max-w-md bg-[#111827]/80 backdrop-blur-lg border border-slate-700 rounded-2xl shadow-2xl shadow-black/30 p-10 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <Input icon={<ShieldCheck size={18} />} type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email de Staff" required />
            <Input icon={<Lock size={18} />} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Contraseña" required />
            <Input icon={<Fingerprint size={18} />} type="text" value={twoFactorCode} onChange={e => setTwoFactorCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="Token de Seguridad 2FA" required />
